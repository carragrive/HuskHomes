...
public class BaseHuskHomesAPI {
     ...
     * generated by the current {@link RandomTeleportEngine} using the configured default RTP destination and the
     * highest-priority profile whose conditions match the user. Remote destinations require a {@link Broker.Type#REDIS}
     * broker.
     *
     ...
     * @throws IllegalStateException if the user meets no profile's conditions at the default destination
    ...
    public final void randomlyTeleportPlayer(@NotNull OnlineUser user, boolean timedTeleport,
        ...
        final Settings.RtpSettings.Destination destination = settings.getDefault()
                .orElseThrow(() -> new IllegalStateException("No default RTP destination is configured"))
                .getValue();
        destination.selectProfile(user).thenAccept(selected -> {
            final RtpOptions options = selected.orElseThrow(() -> new IllegalStateException(
                    "The user met no RTP profile's conditions for the default destination"
            )).getOptions();
            final String server = destination.getServer().isBlank()
                    ? plugin.getServerName()
                    : destination.getServer();
            if (settings.getMode() == Settings.RtpSettings.Mode.CROSS_SERVER
                    && !server.equals(plugin.getServerName())) {
                ...
                Message.builder()
                        ...
                        .payload(Payload.rtpLocationRequest(destination.getWorld(), options, timedTeleport))
                        .build().send(plugin.getBroker().orElseThrow(), user);
                return;
            }

            final World world = plugin.findWorld(destination.getWorld())
                    .orElseThrow(() -> new IllegalStateException("RTP destination world is not loaded"));
            plugin.getRandomTeleportEngine().getRandomPosition(world, rtpArgs, options).thenAccept(position -> {
                if (position.isEmpty()) {
    ...
    public final void randomlyTeleportPlayerLocally(@NotNull OnlineUser user, boolean timedTeleport,
                                                    @NotNull String... rtpArgs) {
        final RtpOptions options = plugin.getSettings().getRtp().getDefault()
                .map(entry -> entry.getValue().getProfiles().get("default"))
                .map(Settings.RtpSettings.Profile::getOptions)
                .orElseGet(RtpOptions::new);
        plugin.getRandomTeleportEngine()
                .getRandomPosition(user.getPosition().getWorld(), rtpArgs, options)
                .thenAccept(position -> {
                    if (position.isEmpty()) {
                        throw new IllegalStateException("Random teleport engine returned an empty position");
                    }
     ...
     * @return The MiniMessage-formatted {@link Component}
