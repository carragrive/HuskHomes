...
### LuckPerms
[LuckPerms](https://luckperms.net) is the recommended permissions plugin for Paper servers. Follow the LuckPerms setup guide to get started. You can then use the `/lp editor` to assign the permissions detailed on this page to groups. A lot of HuskHomes' permissions are enabled for default users ([see below](#without-a-permissions-plugin)). You can set permissions to "false" to revoke a node from a group.
